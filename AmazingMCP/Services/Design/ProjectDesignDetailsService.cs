...
public class ProjectDesignDetailsService(
    ...
    IOptions<ProjectDesignOptions> options) : IProjectDesignDetailsService
{
    readonly ProjectDesignOptions _options = options.Value;
        ...
        CancellationToken ct = default)
    {
        ...
        return Format(depMap, forNamespaces, includeDependencyUsage, includeImplementations);
    }
    ...
    {
        var patterns = forNamespaces.Select(wildcardFactory.CreateGlob).ToList();
        ...
        foreach (var abstraction in matchedAbstractions)
        {
            ...
            {
                var summary = abstraction.XmlDocSummary.Length > _options.DetailsXmlDocSummaryMaxLength
                    ? abstraction.XmlDocSummary[.._options.DetailsXmlDocSummaryMaxLength] + " <<truncated>>"
        ...
        if (result.Length > _options.DetailsOutputMaxLength)
            return result[.._options.DetailsOutputMaxLength] + TruncationSuffix;
